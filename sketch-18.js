  ...
  return ({ context, width, height }) => {
    ...
    for (i = 0; i < 20; i++) {
      for (j = 0; j < 20; j++) {
        ...
        DotToMouseY = mouseY - dotY;
        DotToMouseX = mouseX - dotX;
        ...
        const distanceToMouse = Math.sqrt(
          DotToMouseX * DotToMouseX + DotToMouseY * DotToMouseY
        );

        context.fillStyle = `hsl(${(distanceToMouse / 300) * 360}, 100%, 80%)`;
        ...
        context.beginPath();
        context.moveTo(0, 0);
        context.lineTo(0, 0.08 * distanceToMouse);
        context.stroke();
